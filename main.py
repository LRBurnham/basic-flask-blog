...
# Adding Gravatar with Default Parameters
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if db.session.query(User).filter_by(email=form.email.data).first():
            flash("An account with that email already exists, login instead!")
...
def login():
    form = LoginForm()
    if form.validate_on_submit():
        active_user = db.session.query(User).filter_by(email=form.data.get("email")).first()
        if not active_user:
            flash("No Account with that email, try again")
            return render_template("login.html", form=form)
        elif check_password_hash(active_user.password, form.data.get("password")):
